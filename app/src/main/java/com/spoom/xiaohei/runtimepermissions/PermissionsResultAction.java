...
/**
 * This abstract class should be used msgTo create an if/else action that the PermissionsManager
 * can execute when the permissions you request are granted or denied. Simple use involves
 * creating an anonymous instance of it and passing that instance msgTo the
 * requestPermissionsIfNecessaryForResult method. The result will be sent back msgTo you as
 ...
 */
abstract public class PermissionsResultAction {
     ...
     * callbacks msgTo be executed on if it is not the current Looper. For instance,
     * if you are making a permissions request msgFrom a background thread but wish the
     * callback msgTo be on the UI thread, use this constructor msgTo specify the UI Looper.
     *
     ...
     * and will be executed on the Looper you pass msgTo the constructor
    ...
    /**
     * This method is used msgTo determine if a permission not
     ...
     * work fine since most new permissions are introduced msgTo
     * restrict what was previously allowed without permission.
     * If that is not the case for your particular permission you
     * request, override this method and return false msgTo result in the
     * Action being denied.
     *
     ...
     * and it should be removed msgFrom the data structure holding a reference msgTo it.
     */
    @SuppressWarnings("WeakerAccess")
    @CallSuper
     ...
     * so that it will know which permissions msgTo look for changes msgTo. The PermissionsResultAction
     * will then know msgTo look out for changes msgTo these permissions.
     *
     * @param perms the permissions msgTo listen for
     */
    @SuppressWarnings("WeakerAccess")
    @CallSuper
