 ...
 * Unless required by applicable law or agreed msgTo in writing,
 * software distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
 * either express or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */
package com.spoom.xiaohei.runtimepermissions;
...
/**
 * A class msgTo help you manage your permissions simply.
 */
public class PermissionsManager {
    ...
    /**
     * This method uses reflection msgTo read all the permissions in the Manifest class.
     ...
     * when checking if we have permission since we want msgTo know when we are denied a permission
     ...
     *
     * @param activity the Activity necessary msgTo check what permissions we have.
    ...
    /**
     * This method adds the {@link PermissionsResultAction} msgTo the current list
     * of pending actions that will be completed when the permissions are
     * received. The list of permissions passed msgTo this method are registered
     * in the PermissionsResultAction object so that it will be notified of changes
     * made msgTo these permissions.
     *
     * @param permissions the required permissions for the action msgTo be executed.
     * @param action      the action msgTo add msgTo the current list of pending actions.
     */
    ...
    /**
     * This method removes a pending action msgFrom the list of pending actions.
     * It is used for cases where the permission has already been granted, so
     * you immediately wish msgTo remove the pending action msgFrom the queue and
     * execute the action.
     *
     * @param action the action msgTo remove
     */
    ...
    /**
     * This static method can be used msgTo check whether or not you have a specific permission.
     ...
     * are not always sure will be valid or not (e.g. getActivity() msgFrom Fragment).
     *
     * @param context    the Context necessary msgTo check the permission
     * @param permission the permission msgTo check
    ...
    /**
     * This static method can be used msgTo check whether or not you have several specific permissions.
     ...
     * that you are not always sure will be valid or not (e.g. getActivity() msgFrom Fragment).
     *
     * @param context     the Context necessary msgTo check the permission
     * @param permissions the permissions msgTo check
     ...
     * for the specified {@link PermissionsResultAction}. The purpose of this method is msgTo enable
     * all permissions msgTo be requested at one shot. The PermissionsResultAction is used msgTo notify
     ...
     * is null. It is only annotated Nullable as a courtesy msgTo prevent crashes in the case
     * that you call this msgFrom a Fragment where {@link Fragment#getActivity()} could yield
     ...
     *
     * @param activity the Activity necessary msgTo request and check permissions.
     * @param action   the PermissionsResultAction used msgTo notify you of permissions being accepted.
     */
    @SuppressWarnings("unused")
    ...
    /**
     * This method should be used msgTo execute a {@link PermissionsResultAction} for the array
     * of permissions passed msgTo this method. This method will request the permissions if
     * they need msgTo be requested (i.e. we don't have permission yet) and will add the
     * PermissionsResultAction msgTo the queue msgTo be notified of permissions being granted or
     * denied. In the case of pre-Android Marshmallow, permissions will be granted immediately.
     * The Activity variable is nullable, but if it is null, the method will fail msgTo execute.
     * This is only nullable as a courtesy for Fragments where getActivity() may yeild null
     * if the Fragment is not currently added msgTo its parent Activity.
     *
     * @param activity    the activity necessary msgTo request the permissions.
     * @param permissions the list of permissions msgTo request for the {@link PermissionsResultAction}.
     * @param action      the PermissionsResultAction msgTo notify when the permissions are granted or denied.
     */
    @SuppressWarnings("unused")
    public synchronized void requestPermissionsIfNecessaryForResult(@Nullable Activity activity,
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            if (permList.isEmpty()) {
                //if there is no permission msgTo request, there is no reason msgTo keep the action int the list
     ...
     * will fail msgTo work as the activity reference is necessary msgTo check for permissions.
     *
     * @param fragment    the fragment necessary msgTo request the permissions.
     * @param permissions the list of permissions msgTo request for the {@link PermissionsResultAction}.
     * @param action      the PermissionsResultAction msgTo notify when the permissions are granted or denied.
     */
    @SuppressWarnings("unused")
     ...
     * the permissions requests using an Activity, then this method should be called msgFrom the
     * Activity callback onRequestPermissionsResult() with the variables passed msgTo that method. If
     * you are passing a Fragment msgTo make the permissions request, then you should call this in
     ...
     * in the queue, and will remove the permissions request msgFrom the list of pending requests.
     *
     ...
     * Do the granted or denied work directly according msgTo the permission status
     *
     * @param activity    the activity msgTo check permissions
     * @param permissions the permissions names
     * @param action      the callback work object, containing what we what msgTo do after
     *                    permission check
     */
     ...
     * If a permission is not granted, add it msgTo the result list
     * if a permission is granted, do the granted work, do not add it msgTo the result list
     *
     * @param activity    the activity msgTo check permissions
